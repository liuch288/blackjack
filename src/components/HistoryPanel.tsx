        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            className="px-4 py-2 bg-red-500/10 hover:bg-red-500/20 border border-red-500/30
              rounded-lg text-sm font-medium text-red-300 transition-all
              ...
              <tbody>
                {pageData.map((r, i) => (
                  <tr key={r.id} className={`border-t border-white/5 ${i % 2 === 0 ? 'bg-white/[0.02]' : ''}`}>
                    <td className="px-3 py-3 text-white/30 text-xs">{r.id}</td>
                    ...
                    <td className="px-3 py-3 text-center">
                      <span className={`text-xs px-2 py-0.5 rounded ${
                        r.handType === 'Hard' ? 'bg-orange-500/15 text-orange-300' :
                        r.handType === 'Soft' ? 'bg-blue-500/15 text-blue-300' :
                        'bg-purple-500/15 text-purple-300'
                      }`}>
                        {r.handType === 'Hard' ? '硬' : r.handType === 'Soft' ? '软' : '对'}
                      </span>
                    </td>
                    <td className="px-3 py-3 text-center text-green-400 font-medium">{actionShortLabel(r.correctAction)}</td>
                    ...
                    <td className="px-3 py-3 text-center font-mono text-xs">
                      <span className={r.correctEv >= 0 ? 'text-green-400' : 'text-red-400'}>
                        {r.correctEv >= 0 ? '+' : ''}{r.correctEv.toFixed(3)}
                      </span>
                    </td>
                    <td className="px-3 py-3 text-center font-mono text-xs">
                      <span className={r.userEv >= 0 ? 'text-green-400' : 'text-red-400'}>
                        {r.userEv >= 0 ? '+' : ''}{r.userEv.toFixed(3)}
                      </span>
                    </td>
                    <td className="px-3 py-3 text-center font-mono text-xs text-yellow-400">
                      {r.evDiff.toFixed(3)}
                    </td>
                  </tr>
                ))}
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-center gap-2 pt-4">
              <button
                ...
                className="px-3 py-1.5 bg-white/10 rounded-lg text-sm disabled:opacity-30"
              >
                ←
              </button>
