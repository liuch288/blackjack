...
function bestAction(actions: Record<string, number>, rules?: GameRules, handTotal?: number, dealerUp?: string): string {
  const filtered = filterActions(actions, rules, handTotal, dealerUp);
  return bestActionFromFiltered(filtered);
}
...
function cellClass(act: string): string {
  return ACT_COLORS[act] ? ACT_COLORS[act].replace('bg-', 'bg-') + '/25' : '';
}
...
function CellPopover({ data, onClose }: { data: PopoverData; onClose: () => void }) {
  ...
  return (
    <div
      className="fixed bg-gray-900 border border-white/15 rounded-xl p-4 shadow-2xl z-50 min-w-[200px]"
    ...
    >
      <div className="flex items-center justify-between mb-3">
        ...
        <button onClick={onClose} className="text-white/30 hover:text-white/70 text-lg leading-none ml-2 cursor-pointer">&times;</button>
      </div>
        ...
        {entries.map(([act, ev]) => {
          ...
          return (
            <div key={act} className="flex items-center justify-between gap-4">
              <span className="text-xs text-white/50">{actionLabel(act as Action)}</span>
...
export function EvViewer() {
  ...
  const handleCellClick = (e: React.MouseEvent, acts: Record<string, number>, handLabel: string, dealerUp: string) => {
    ...
    const popW = 200; // min-w-[200px]
    const popH = Object.keys(acts).length * 28 + 64; // 估算高度
  ...
  return (
    <div className="px-4 py-6 max-w-5xl mx-auto">
      ...
      <div className="flex flex-wrap gap-4 text-xs text-white/50 mb-4">
        {[
          { act: 'Hit', cls: 'bg-blue-500/50' } as const,
          { act: 'Stand', cls: 'bg-green-500/50' } as const,
          { act: 'Double', cls: 'bg-amber-500/50' } as const,
          { act: 'Split', cls: 'bg-purple-500/50' } as const,
          ...(rules.surrender !== 'none' ? [{ act: 'Surrender' as const, cls: 'bg-pink-500/50' as const }] : []),
        ].map(({ act, cls }) => (
  ...
  return hands.map(h => (
    <tr key={h} className="border-t border-white/5">
      ...
      {UPCARDS.map(u => {
        ...
        if (!rawActs || Object.keys(rawActs).length === 0) return <td key={u} className="px-3 py-3 text-center text-white/10">—</td>;
        const filtered = filterActions(rawActs, rules, parseInt(h), u);
        const best = bestActionFromFiltered(filtered);
        return (
          <td
            key={u}
            className={`px-2 py-2 text-center ${cellClass(best)} cursor-pointer hover:ring-1 hover:ring-white/20`}
            onMouseDown={e => onCellClick(e, filtered, `硬手 ${h}`, u)}
          >
            <div className="font-bold text-sm">{actionLabel(best as any)}</div>
            <div className={`text-xs mt-0.5 ${filtered[best] >= 0 ? 'text-amber-400' : 'text-white/40'}`}>
